...
function BarGroup(props) {
  ...
  let width = widthScale(props.d.shelternum)
  let yMid = props.barHeight * 0.5
  
  return <g className="bar-group">
    <text className="name-label" x="-6" y={yMid} alignmentBaseline="middle" >{props.d.state}</text>
    <rect y={barPadding * 0.5} width={width} height={props.barHeight - barPadding} fill={barColour} />
    <text className="value-label" x={width- 8} y={yMid} alignmentBaseline="middle" >{props.d.shelternum}</text>
  </g>
}

class NumberShelter extends React.Component {
    constructor(props) {
      super(props);
      this.state = {
        isToggleOn: true,
      ...
      fetch('http://localhost:8080/test/numbershelter')
    ...
    render() {
      ...
      return (
        <div>
          <h> Which state has the most number of animal shelters?</h>
          <button onClick={this.handleClick}>
            {this.state.isToggleOn ? 'Hide' : 'Show'}
          </button>
          <svg width="800" height="300" style={{display: this.state.display}}>
            <g className="container">
              <text className="title" x="10" y="30">Number of animal shelters</text>
